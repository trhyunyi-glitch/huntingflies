...
with col2:
    ...
    if hit_fly and hit_tank:
        ...
        st.warning("⚠️ **반쪽짜리 성공?** 파리는 맞췄지만, 포탄이 전차에서 발사되지 않았어요! 전차를 이차함수의 x절편 위치로 이동시켜 보세요.")
...
1. 파리의 y 좌표가 항상 양수(제1, 2사분면)에 있습니다. 그렇다면 포탄이 위로 볼록해야 할까요, 아래로 볼록해야 할까요? 계수 a의 부호는 무엇이어야 할지 생각해 보세요.
2. 전차의 위치는 이 이차함수 그래프의 무엇을 의미할까요? (힌트: y=0일 때의 x값, 즉 **x절편**과 어떤 관계가 있을지 고민해 보세요!)
""")
